...
struct Puzzle {
    containers: Vec<u32>,
    count: u32,
    min_container_count: u32,
}
impl Puzzle {
    fn init_from_string(&mut self, input_string: &String) {
        self.min_container_count = u32::MAX;
        for line in input_string.lines() {
            let val = sscanf::sscanf!(line, "{}", u32).unwrap();
            self.containers.push(val);
        }
    }
    fn fill_next_container(&mut self, index: usize, volume_left: u32, container_count: u32) {
        if container_count > self.min_container_count {
            return;
        }
        if volume_left == 0 {
            if container_count < self.min_container_count {
                self.min_container_count = container_count;
                self.count = 1;
            } else if container_count == self.min_container_count {
                self.count += 1;
            }
            return;
        }
        ...
        if volume_left >= self.containers[index] {
            self.fill_next_container(
                index + 1,
                volume_left - self.containers[index],
                container_count + 1,
            );
        }
        //
        ...
        //
        self.fill_next_container(index + 1, volume_left, container_count);
    }
}
...
fn solve_puzzle(input_string: &String, volume: u32) -> u32 {
    ...
    puzzle.fill_next_container(0, volume, 0);
