...
impl World {
    ...
    fn next_hop(&mut self, cur: usize, avail: &mut VecDeque<usize>, dist: u32) {
        //
        // We have already found a shorter path, turn back
        //
        if dist > self.shortest_path {
            return;
        }
        //
        // Are we at the end?
        //
        if avail.is_empty() {
            self.shortest_path = min(self.shortest_path, dist);
        }
        let count = avail.len();
        for _i in 0..count {
...
fn main() {
    ...
    //
    let expected_sample_output = 605;
