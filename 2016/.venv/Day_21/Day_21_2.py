...
def reverse_rotate(string, direction, steps):
    if direction == "left":
        return rotate(string, "right", steps)
    return rotate(string, "left", steps)
...
def reverse_rotate_based_on_position(string, letter):
    for index in range(len(string)):
        test_string = rotate(string, "Right", index)
        result_string = rotate_based_on_postion(test_string, letter)
...
def execute_instruction(line, string):
    ...
    if match := swap_pos_re.search(line):
        ...
        string = reverse_rotate(string, match.groups()[
                                0], int(match.groups()[1]))
    elif match := rotate_pos_re.search(line):
        string = reverse_rotate_based_on_position(string, match.groups()[0])
    elif match := reverse_sub_re.search(line):
        string = reverse_substring(string, int(
            match.groups()[0]), int(match.groups()[1]))
    elif match := move_re.search(line):
        string = move(string, int(match.groups()[1]), int(match.groups()[0]))
    else:
        print("!!!!! ", line)
        assert (False)
...
print(execute("test.txt", "decab"))
assert (execute("input.txt", "aefgbcdh") == "abcdefgh")
print("Result for part 2 : ", execute("input.txt", "fbgdceah"))
