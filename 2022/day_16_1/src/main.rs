...
impl Cave {
    ...
    fn make_next_move(
        ...
        self.state_visisted_count += 1;

        // Correct order is DD (2), BB (4), JJ (10), HH (9), EE (6), CC (5)
        if time_left == 0 {
            self.best_score = max(self.best_score, score_so_far);
            return;
        }
        ...
        if current_flow > 0 {
            ...
            //
            new_time_left -= 1;
            let score_for_this_valve = current_flow * new_time_left;
            new_score += score_for_this_valve;
        }

        self.best_score = max(self.best_score, new_score);
        if self.open_valves.len() == self.interesting_valves.len() {
            self.best_score = max(self.best_score, new_score);
            return;
        }
        let new_flow_left = flow_left - current_flow;
        let theor_max = new_flow_left * new_time_left;

        if new_score + theor_max < self.best_score {
            return;
        }

        for i in 0..self.interesting_valves.len() {
            let (id, flow) = self.interesting_valves.get(i).unwrap().clone();
            if self.open_valves.contains(&id) {
                continue;
            }
            assert!(id != current_valve);
            let time = *self.distances.get(&(current_valve, id)).unwrap();
            if time > new_time_left {
                continue;
            }
            assert!(time > 0);
            self.open_valves.insert(id);
            self.make_next_move(id, flow, new_score, new_flow_left, new_time_left - time);
            self.open_valves.remove(&id);
        }
    }
    ...
    fn find_solution(&mut self, starting_valve: &String, time_limit: u32) -> u32 {
        ...
        self.make_next_move(*start_id, 0, 0, self.max_flow, time_limit);
        println!("{}", self.state_visisted_count);
...
fn solve_puzzle(input_string: &String) -> u32 {
    ...
    let result = cave.find_solution(&start.to_string(), 30);
    println!("{}", cave.best_score);
    //
    // Real result is 2056
    //
    result
...
fn main() {
    ...
    //
    let expected_sample_output = 1651;
