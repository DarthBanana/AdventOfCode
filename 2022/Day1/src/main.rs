...
fn main() {
    ...
    let input_result = std::fs::read_to_string(std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("sample.txt"));
    let test_input_string = input_result.unwrap();
    let input_result = std::fs::read_to_string(std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("input.txt"));
