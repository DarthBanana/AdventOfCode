...
impl Puzzle {
    ...
    fn find_path(&self, pos: Coord, minute: i64, state: &mut State) {
        ...
        if pos == self.end_coord {
            state.shortest_path = min(minute, state.shortest_path);
            return;
        }
        let distance_to_end = (self.end_coord.y - pos.y) + (self.end_coord.x - pos.x);
        if minute + distance_to_end > state.shortest_path {
            return;
        }

        let moves = self.get_moves_for_minute(pos, minute + 1);
        for m in moves {
...
fn solve_puzzle(input_string: &String) -> i64 {
    ...
    puzzle.find_path(puzzle.start_coord, 0, &mut state);
